...
def generate_reads(read_seq, read_pos, ref_alleles, alt_alleles, i):
    ...
    ##### START HERE: CODE ADDED BY MAHEETHA AND EDITED BY MAHEETHA ####
    ...
    ##### END HERE: CODE ADDED BY MAHEETHA AND EDITED BY MAHEETHA #####
...
def write_pair_fastq(fastq_file1, fastq_file2, orig_read1, orig_read2,
    ...
    for pair in new_pairs:
        ...
        fastq_file1.write(bytes("@%s\n%s\n+%s\n%s\n" %
                          (name, pair[0], name, orig_read1.qual) ,'UTF-8')) ### ADDED THE UTF-8

        rev_seq = util.revcomp(pair[1])
        fastq_file2.write(bytes("@%s\n%s\n+%s\n%s\n" %
                          (name, rev_seq, name, orig_read2.qual), 'UTF-8')) #### ADDED THE UTF-8
    ...
    for read in files.input_bam:
        ...
        if (cur_tid is None) or (read.tid != cur_tid):
            ...
            if files.snp_tab_h5:
                ...
                snp_filename = "%s/%s.snps.txt" % (files.snp_dir, cur_chrom)
                sys.stderr.write("reading SNPs from file '%s'\n" % snp_filename)
...
def process_single_read(read, read_stats, files, snp_tab, max_seqs,
    ...
    if len(snp_idx) > 0:
        ref_alleles = snp_tab.snp_allele1[snp_idx]
        ref_alleles = ref_alleles.decode('UTF-8') #### ADD THE UTF-8
        alt_alleles = snp_tab.snp_allele2[snp_idx]
        alt_alleles = alt.alleles.decode('UTF-8') #### ADD THE UTF-8
